...
**5. 프로그래머스 문제 [42587](https://programmers.co.kr/learn/courses/30/lessons/42587)**
...
고유 인덱스를 저장할 waiting이라는 리스트를 만든다.

그 후에 프린트할 것이 남아있다면 while문을 순회하면서 답을 찾아낸다.

현재 프린트해야할 첫번째 것이 max값보다 작다면 뒤에 append하고, max값이라면 pop한다.

waiting도 priorities와 같이 pop, append 해주고 waiting[0]이 location의 값과 같다면 while문을 끝낸다.
